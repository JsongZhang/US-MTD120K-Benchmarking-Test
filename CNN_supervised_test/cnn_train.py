...
def main():
    ...
    data_root = os.path.abspath(os.path.join(os.getcwd(), "../"))  # get UMTD root path
    image_path = os.path.join(data_root, "supervised_work", "clc")  # UMTD set path
    assert os.path.exists(image_path), "{} path does not exist.".format(image_path)

    train_dataset = datasets.STL10('./stl-10/', split='train', download=True, transform=data_transform["train"])
    ...
    net.fc = nn.Linear(in_channel, 28)
    net.to(device)
